import { t } from "../js/i18n.js";

function isActive(path){
    return window.location.pathname === path 
        ? "nav-link__active"
        : "";
}

export default function Header(){
    return `
        <header class="split">
            <button class="mobile" id="sidebar_btn"> <i class="fa-solid fa-bars"></i> </button>
            <div class="navbar -vnav">
                <a href="/" class="navbar__links -vbuttons ${isActive("/")}">${t("pages.home")}</a>
                <a href="/skills" class="navbar__links -vbuttons ${isActive("/skills")}">${t("pages.skills")}</a>
                <a href="/projects" class="navbar__links -vbuttons ${isActive("/projects")}">${t("pages.projects")}</a>
                <a href="/contact" class="navbar__links -vbuttons ${isActive("/contact")}">${t("pages.contact")}</a>
            </div>
            <div class="lang container">
                <button class="lang-switch btn btn--secondary btn--icon">
                    ${t("lang.icon")}
                    ${t("lang.name")}
                </button>
                <div class="container stack lang-selector">
                    <button class="btn btn--secondary btn--icon"><i class='icon usa-icon'></i>EN</button>
                    <button class="btn btn--secondary btn--icon"><i class='icon mex-icon'></i>ES</button>
                </div>
            </div>
        </header>
    `
}